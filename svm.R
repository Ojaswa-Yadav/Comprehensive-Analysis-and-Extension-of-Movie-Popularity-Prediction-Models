...
rating_label_mapping <- c('1' = 'Average', '2' = 'Excellent', '3' = 'Good', '4' = 'Poor')  # Example mapping
  ...
  # Train the SVM model
  model <- svm(train_data_numeric, train_target, type = 'C-classification', kernel = 'polynomial')

  # Make sure the test data has the same transformations
  test_data_numeric <- make_numeric(test_data[, !(names(test_data) %in% target_variable)])

  # Predict on the test data
  predictions <- predict(model, test_data_numeric)

  # Return the predictions as a factor with the correct levels
...
svm_classification <- function(train_data, test_data, target_variable, label_mapping) {
...
predictions_gross_conv <- svm_classification(train_conv, test_conv, "GrossLabel", gross_label_mapping)
predictions_gross_social <- svm_classification(train_social, test_social, "GrossLabel", gross_label_mapping)
predictions_rating_conv <- svm_classification(train_conv, test_conv, "RatingLabel", rating_label_mapping)
predictions_rating_social <- svm_classification(train_social, test_social, "RatingLabel", rating_label_mapping)
predictions_gross_full <- svm_classification(train_full, test_full_features, "GrossLabel", gross_label_mapping)
predictions_rating_full <- svm_classification(train_full, test_full_features, "RatingLabel", rating_label_mapping)
