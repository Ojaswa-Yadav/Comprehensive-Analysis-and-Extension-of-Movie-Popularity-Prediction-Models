...
train_social <- data[train_index, c("Sentiment", "Views", "Likes", "Dislikes", "Comments", "Aggregate Followers")]
test_social <- data[-train_index, c("Sentiment", "Views", "Likes", "Dislikes", "Comments", "Aggregate Followers")]
...
```{r}
linear_regression_classification <- function(train_data, test_data, label) {
  model <- lm(as.formula(paste(label, "~ .")), data = train_data)
...
pred_gross_conv <- linear_regression_classification(
  cbind(train_gross, train_conv), test_conv, "GrossLabel")

pred_gross_social <- linear_regression_classification(
  cbind(train_gross, train_social), test_social, "GrossLabel")

pred_gross_both <- linear_regression_classification(
  cbind(train_gross, train_both), test_both, "GrossLabel")

pred_rating_conv <- linear_regression_classification(
  cbind(train_rating, train_conv), test_conv, "RatingLabel")

pred_rating_social <- linear_regression_classification(
  cbind(train_rating, train_social), test_social, "RatingLabel")

pred_rating_both <- linear_regression_classification(
...
```{r}
model_gross_conv <- lm(GrossLabel~ ., data = cbind(train_gross, train_conv))
model_gross_social <- lm(GrossLabel~ ., data = cbind(train_gross, train_social))
model_gross_both <- lm(GrossLabel~ ., data = cbind(train_gross, train_both))
model_rating_conv <- lm(RatingLabel~ ., data = cbind(train_rating, train_conv))
model_rating_social <- lm(RatingLabel~ ., data = cbind(train_rating, train_social))
model_rating_both <- lm(RatingLabel~ ., data = cbind(train_rating, train_both))
```

```{r}
# summary(model_gross_conv)
summary(model_gross_social)
# summary(model_gross_both)
# summary(model_rating_conv)
# summary(model_rating_social)
# summary(model_rating_both)
```
